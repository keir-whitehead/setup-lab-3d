...
export default function MachineCard({ machine, isSelected, onSelect, onToggle, onUpdate }: MachineCardProps) {
  ...
  const handleChipChange = (newChip: string) => {
    ...
    const newBandwidth = config.bandwidthMap[newRam] ?? '100 GB/s';
    const newCpu = CHIP_CPU_MAP[newChip] ?? machine.cpu;
    onUpdate(machine.id, {
      chip: newChip,
      ram: newRam,
      gpu: newGpu,
  ...
  const handleRamChange = (newRam: string) => {
    ...
    const newBandwidth = chipConfig.bandwidthMap[newRam] ?? machine.bandwidth;
    onUpdate(machine.id, { ram: newRam, bandwidth: newBandwidth });
  };

  const handleGpuChange = (newGpu: string) => {
    if (!onUpdate) return;
    onUpdate(machine.id, { gpu: newGpu });
  };
